...
<head>
	...
	<?php  $validacion = $this->session->flashdata('carga');
		if($validacion=='true'){
			?>
				<style type="text/css">.tabla_roles, .pagination{display: table;}</style>
			<?php
		}else{
			$listarRoles='';
			?>
				<style type="text/css">.tabla_roles, .pagination{display: none;}</style>
			<?php
		}
	?>
</head>
		...
		<div class="row-fluid">
						...
						<form class="form-horizontal" method="post" action="registrar">
							<fieldset>
							 ...
							 <div class="form-actions">
								...
								<button type="submit" class="btn btn-primary">Listar Roles</button>
							  </div>
							</fieldset>
						  </form>
			...
			<div class="row-fluid sortable">	
				<div class="box span12">
					...
					<div class="box-content">
						<form action="consultarRoles">
							<button type="submit" class="btn btn-primary">Consultar Roles</button>
						</form>
						<table class="table table-bordered table-striped table-condensed tabla_roles">
							  <thead>
								  <tr>
									  <th>Rol</th>
									  <th>Permisos</th>
										...
										if($listarRoles!=''){
									        ...
									        foreach ($listarRoles as $valor) {
									            ...
									            if($valor['nombre1']!=$nombre){
									               ?><tr><td class="id_rol"><?php echo $valor['id'];?></td><td class="rol"><?php echo $valor['nombre1'];?></td><td class="permisos"><?php
									            }
									            echo  $valor['nombre2'].",";  
						  ...
						  <fieldset>
							  <div class="control-group">
							   ...
							   <div class="control-group">
									...
									<div class="controls">
									  <select id="selectPermisos" multiple data-rel="chosen">
										<option value="1">Insertar</option>
										<option value="2" selected>Consultar</option>
										<option value="3">Modificar</option>
										<option value="4">Eliminar</option>
